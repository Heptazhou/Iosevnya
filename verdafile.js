...
const SampleImagesPre = task(`sample-images:pre`, async (target) => {
	const [sans, slab, aile, etoile] = await target.need(GroupContents`iosevka`, GroupContents`iosevka-slab`, GroupContents`iosevka-aile`, GroupContents`iosevka-etoile`, SnapShotStatic("index.js"), SnapShotStatic("get-snap.js"), SnapShotJson, SnapShotCSS, SnapShotHtml, de`images`, de(SNAPSHOT_TMP))
...
const TakeSampleImages = task(`sample-images:take`, async (target) => {
	...
	await Promise.all(taskLists.map((file, i) => Delay(i * 4000).then(() => cd(SNAPSHOT_TMP).run("npx", "electron", "get-snap.js", "../../images", file))))
})
const ScreenShot = file.make(
	(img) => `images/${img}.png`,
	async (target, out) => {
		await target.need(TakeSampleImages)
		await run("magick", ...[`${out.dir}/${out.name}.black.png`, `${out.dir}/${out.name}.white.png`], ..."( -clone 0-1 -fx u-v+1 )".split(" "), ..."( -clone 0,2 -compose DivideSrc -composite )".split(" "), ..."( -clone 3,2 -alpha Off -compose CopyOpacity -composite )".split(" "), ...["-delete", "0-3", out.full])
...
phony(`release`, async (target) => {
	...
	let goals = []
	for (const [cgr, plan] of Object.entries(collectPlans)) if (plan.inRelease) goals.push(ReleaseGroup(cgr))
...
const UtilScriptFiles = computed("util-script-files", async (target) => {
	const [js, ejs, md] = await target.need(ScriptsUnder("js", "utility"), ScriptsUnder("ejs", "utility"), ScriptsUnder("md", "utility"))
	return [...js, ...ejs, ...md]
})
const SnapshotTemplateFiles = computed("snapshot-templates", async (target) => {
	const [js, ejs, md] = await target.need(ScriptsUnder("js", "snapshot-src"), ScriptsUnder("ejs", "snapshot-src"), ScriptsUnder("md", "snapshot-src"))
	return [...js, ...ejs, ...md]
})
