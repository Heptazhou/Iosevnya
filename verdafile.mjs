...
const FontInfoOf = computed.group("metadata:font-info-of", async (target, fileName) => {
	...
	return {
		...
		hintReference: !bp["metric-override"] && hintReferenceSuffix !== fi0.suffix ? makeFileName(fi0.prefix, hintReferenceSuffix) : null,
...
const DistUnhintedTTF = file.make(
	...
	async (target, out, gr, fn) => {
		...
		if (fi.spacingDerive) {
			...
			const [deriveFrom] = await target.need(DistUnhintedTTF(spD.prefix, spD.fileName), de(charMapDir))
			...
			const [comps] = await target.need(CompositesFromBuildPlan, de(charMapDir), de(SHARED_CACHE))
...
const DistHintedTTF = file.make(
	...
	async (target, out, gr, fn) => {
		...
		if (fi.spacingDerive) {
			...
			const [deriveFrom] = await target.need(DistHintedTTF(spD.prefix, spD.fileName), de(charMapDir))
			echo.action(echo.hl.command(`Create TTF`), out.full)
			await silently.node(`font-src/derive-spacing.mjs`, {
				i: deriveFrom.full,
				oNoGc: noGcTtfPath,
...
async function getCollectPlans(target, rawCollectPlans) {
	...
	for (const gr of allCollectableGroups) amendedRawCollectPlans[`sgr-` + gr] = { release: true, isAmended: true, from: [gr] }
...
const GroupTtfZip = file.make(
	(gr, version, unhinted) => `${ARCHIVE_DIR}/${formatSuffix("ttf", unhinted)}-${gr}-${version}.zip`,
	async (target, out, gr, _version_, unhinted) => {
		await target.need(de`${out.dir}`)
		await target.need(GroupTtfsImpl(gr, unhinted))
		await CreateGroupArchiveFile(`${DIST}/${gr}/${formatSuffix("ttf", unhinted)}`, out, "*.ttf")
	}
)
const GroupWebZip = file.make(
	(gr, version, unhinted) => `${ARCHIVE_DIR}/${formatSuffix("webfont", unhinted)}-${gr}-${version}.zip`,
	async (target, out, gr, _version_, unhinted) => {
		const [plan] = await target.need(BuildPlanOf(gr))
		await target.need(de`${out.dir}`)
		await target.need(GroupWebFontsImpl(gr, unhinted))
		await CreateGroupArchiveFile(`${DIST}/${gr}`, out, `${formatSuffix(gr, unhinted)}.css`, ...plan.webfontFormats.map((format) => formatSuffix(format, unhinted)))
	}
)
...
async function CreateGroupArchiveFile(dir, out, ...files) {
	...
	await cd(dir).silently.run([SEVEN_ZIP, "a"], ["-tzip", "-r", "-mx=9", "-mmt=off"], relOut, ...files)
}
...
const Pages = task(`pages`, async (t) => {
	await t.need(PagesDataExport, PagesFontExport`iosevka`, PagesFontExport`iosevka-slab`, PagesFontExport`iosevka-aile`, PagesFontExport`iosevka-etoile`, PagesFontExport`iosevka-QP`, PagesFontExport`iosevka-QP-slab`, PagesFontExport`iosevka-QPE`, PagesFontExport`iosevka-QPE-slab`)
})
...
const PagesDataExport = task(`pages:data-export`, async (t) => {
	...
	const [cm, cmi, cmo] = await t.need(BuildCM("iosevka", "iosevka-regular"), BuildCM("iosevka", "iosevka-italic"), BuildCM("iosevka", "iosevka-oblique"))
...
const AmendReadme = task("amend-readme", async (target) => {
	await target.need(AmendReadmeFor("README.md"), AmendReadmeFor("doc/stylistic-sets.md"), AmendReadmeFor("doc/character-variants.md"), AmendReadmeFor("doc/custom-build.md"), AmendReadmeFor("doc/language-specific-ligation-sets.md"), AmendReadmeFor("doc/cv-influences.md"), AmendLicenseYear)
})
const AmendReadmeFor = task.make(
	(f) => `amend-readme::for::${f}`,
	async (target, f) => {
		await target.need(Parameters, UtilScripts)
		const [cm, cmi, cmo] = await target.need(BuildCM("iosevka", "iosevka-regular"), BuildCM("iosevka", "iosevka-italic"), BuildCM("iosevka", "iosevka-oblique"))
...
const SampleImagesPre = task(`sample-images:pre`, async (target) => {
	...
	const fontGroups = await target.need(GroupTtfsImpl(`iosevka`, false), GroupTtfsImpl(`iosevka-slab`, false), GroupTtfsImpl(`iosevka-aile`, false), GroupTtfsImpl(`iosevka-etoile`, false))
	const [cm, cmi, cmo] = await target.need(BuildCM("iosevka", "iosevka-regular"), BuildCM("iosevka", "iosevka-italic"), BuildCM("iosevka", "iosevka-oblique"))
...
const ScreenShotImpl = file.make(
	...
	async (target, out, id) => {
		...
		await run(rp.buildOptions.snapshotGeneratorApp, [`${IMAGE_TASKS}/${id}.json`, "-o", out.full])
	}
)
...
const UtilScriptFiles = computed("util-script-files", async (target) => {
	const [mjs, md] = await target.need(ScriptsUnder("mjs", "utility"), ScriptsUnder("md", "utility"))
	return [...mjs, ...md]
})
