...
const AmendReadme = task("amend-readme", async (target) => {
	await target.need(AmendReadmeFor("README.md"), AmendReadmeFor("doc/stylistic-sets.md"), AmendReadmeFor("doc/character-variants.md"), AmendReadmeFor("doc/custom-build.md"), AmendReadmeFor("doc/language-specific-ligation-sets.md"), AmendLicenseYear)
})
...
function validateAndShimBuildPlans(prefix, bp, dWeights, dSlopes, dWidths) {
	...
	if (!bp.slopes && bp.slants) echo.warn(`Build plan for ${prefix} uses legacy "slants" to define slopes. ` + `Use "slopes" instead.`)
